...
vec3 rayleigh(float origin_height, vec3 dir, float atmos_radius, float planet_radius, vec3 sun_dir) {
	...
	intersections.y = min(intersections.y, ray_sphere(vec3(0.0, 0.0, origin_height), dir, planet_radius).x);

	const uint step_count = 8;
	const uint light_step_count = 4;
	...
	float step_size = (intersections.y - intersections.x) / step_count;

	float l = 0.0;
	for(uint i = 0; i != step_count; ++i) {
		vec3 pos = vec3(0.0, 0.0, origin_height) + dir * (l + step_size * 0.5);

		float height = /*length(pos)*/ pos.z - planet_radius;
...

		{
			...
			for(uint j = 0; j != light_step_count; ++j) {
				...
				float light_height = /*length(light_pos)*/ light_pos.z - planet_radius;
