...
ClampingInfo gather_clamping_info(sampler2D color, vec2 uv) {
    ClampingInfo info;
    info.max_color = vec3(0.0);
    info.min_color = vec3(999999.0);

    update_clamping(info, textureOffset(color, uv, ivec2(-1, -1)).rgb);
    update_clamping(info, textureOffset(color, uv, ivec2(-1,  0)).rgb);
    update_clamping(info, textureOffset(color, uv, ivec2(-1,  1)).rgb);

    update_clamping(info, textureOffset(color, uv, ivec2( 0, -1)).rgb);
    update_clamping(info, textureOffset(color, uv, ivec2( 0,  0)).rgb);
    update_clamping(info, textureOffset(color, uv, ivec2( 0,  1)).rgb);
...
void main() {
    ...
    if((flags & clamping_bit) != 0 && sample_valid) {
        // Only clamp when UV delta is > 1 pixel of if the pixel was deoccluded
        /*if(deoccluded || any(greaterThan(abs(prev_uv - uv), inv_size)))*/ {
