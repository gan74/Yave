	...
	if(GImGui)
	{
		...
		settings.WriteAllFn = [](ImGuiContext*, ImGuiSettingsHandler*, ImGuiTextBuffer* buf)
		{
			for (int i = 0; i < g_dock.m_docks.size(); ++i)
			{
				...
				buf->appendf("prev=%d\n", (int)g_dock.getDockIndex(dock.prev_tab));
				buf->appendf("next=%d\n", (int)g_dock.getDockIndex(dock.next_tab));
				buf->appendf("children=%d,%d\n", (int)g_dock.getDockIndex(dock.children[0]), (int)g_dock.getDockIndex(dock.children[1]));
				buf->appendf("parent=%d\n", (int)g_dock.getDockIndex(dock.parent));
				buf->appendf("\n");
			}
		};
		...
		settings.ReadLineFn = [](ImGuiContext*, ImGuiSettingsHandler*, void* entry, const char* line)
		{
			...
			else if (sscanf(line, "prev=%d\n", &x))						dock->prev_tab = g_dock.getDockByIndexSafe(x);
			else if (sscanf(line, "next=%d\n", &x))						dock->next_tab = g_dock.getDockByIndexSafe(x);
			else if (sscanf(line, "children=%d,%d\n", &x, &y)	== 2)	{ dock->children[0] = g_dock.getDockByIndexSafe(x); dock->children[1] = g_dock.getDockByIndexSafe(y); }
			else if (sscanf(line, "parent=%d\n", &x))					dock->parent = g_dock.getDockByIndexSafe(x);
		};

		settings.ReadOpenFn = [](ImGuiContext*, ImGuiSettingsHandler*, const char* id)
		{
			return (void*)g_dock.getDockByIndexSafe(atoi(id));
		};
