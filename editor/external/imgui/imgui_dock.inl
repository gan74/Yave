...
struct DockContext
{
	...
	{
		Dock* dest_dock = getDockUnderMouse();
	...
	Dock* getDockByIndexSafe(int idx)
	{
		if(idx < 0) return nullptr;

		while(m_docks.size() <= idx)
		{
			Dock* new_dock = (Dock*)MemAlloc(sizeof(Dock));
			IM_PLACEMENT_NEW(new_dock) Dock();
			m_docks.push_back(new_dock);
	...
	void load(const char* filename)
	{
		...
		while(!feof(file) && !ferror(file))
		{
			...
			if(!strncmp("[Dock]\n", line, 7)) dock = getDockByIndexSafe(index++);
			if(!dock) continue;
			...
			else if (sscanf(line, "status=%d\n", &x))					dock->status = (Status_)x;
			else if (sscanf(line, "active=%d\n", &x))					dock->active = x;
			else if (sscanf(line, "opened=%d\n", &x))					dock->opened = x;
			else if (sscanf(line, "prev=%d\n", &x))						dock->prev_tab = getDockByIndexSafe(x);
			else if (sscanf(line, "next=%d\n", &x))						dock->next_tab = getDockByIndexSafe(x);
			else if (sscanf(line, "children=%d,%d\n", &x, &y)	== 2)	{ dock->children[0] = getDockByIndexSafe(x); dock->children[1] = getDockByIndexSafe(y); }
			else if (sscanf(line, "parent=%d\n", &x))					dock->parent = getDockByIndexSafe(x);
		}
	}
};
...
void SaveDockContext(const char* filename)
{
	g_dock.save(filename);
}

void LoadDockContext(const char* filename)
{
