...
fn main() {
    ...
    let timer = start();
    let images = inputs.iter().map(|file_name| {
    ...
    write_images_to_file(inputs.first().expect("No input specified"), images, format.as_ref(), quality).unwrap();
...
fn write_images_to_file(file_name: &String, images: Vec<ImageData>, format: &ImageFormat, quality: u8) -> Result<usize> {
    let ref mut writer = BufWriter::new(File::create(file_name.clone() + ".yt").expect("Unable to create output file."));
