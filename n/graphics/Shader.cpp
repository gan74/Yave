...
core::String ShaderBase::parse(core::String src, uint vers) {
	...
	core::String common = "layout(std140, row_major) uniform; const float pi = " + core::String(math::pi) + "; float sqr(float x) { return x * x; }  float saturate(float x) { return clamp(x, 0.0, 1.0); }";
