...
class Camera final : public Transformable<T>, public math::Volume<T>
{
	...
	public:
		...
		Camera() : Transformable<T>(), math::Volume<T>(), zFar(1000), zNear(1), ratio(1), fov(math::pi<T>() / 2) {
			radius = zFar;
			computeViewMatrix();
		...
		void setForward(math::Vec<3, T> f, RotationAxis r = Up) {
			f.normalize();
			if(fabs(f.dot(forward)) == 1.0) {
				return;
			}
			math::Vec<3, T> s = (r == Side ? math::Vec<3, T>(0, -1, 0) : math::Vec<3, T>(0, 0, -1)) ^ f;
