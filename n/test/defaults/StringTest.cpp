...
class StringTest : public TestTemplate<StringTest>
{
	...
	private:
		void basicTests() {
			...
			test(str.endsWith("pFlapFlip"), true, "endWith test failed");
			test(str.endsWith("Flap"), false, "endWith test 2");
			test(str.beginsWith("gna ?"), false, "beginWith test failed");
			test(str.beginsWith("Floup"), true, "beginWith test 2");
			test(str.endsWith(""), true, "endWith \"\" test failed");
