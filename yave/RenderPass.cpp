...
static vk::AttachmentReference create_attachment_reference(ImageUsage usage, usize index) {
	return vk::AttachmentReference()
		...
		.setLayout(usage.vk_image_layout())
	;
}
...
static vk::RenderPass create_renderpass(DevicePtr dptr, ImageFormat depth_format, std::initializer_list<ImageFormat> color_formats, ImageUsage color_usage) {
	auto attachments =
			core::range(color_formats).map([=](const auto& format) { return create_attachment(format, color_usage.vk_image_layout()); }).collect<core::Vector>() +
			create_attachment(depth_format, ImageUsage(ImageUsageBits::DepthBit).vk_image_layout());
